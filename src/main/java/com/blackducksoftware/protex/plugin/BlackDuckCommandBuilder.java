...
public abstract class BlackDuckCommandBuilder {
	...
	public static class ProcessOptions {
		...
		private ProcessOptions(final boolean debug, final long maxHeapSize) {
			this.debug = debug;
		...
		public String getProxyHost(final String protocol, final Proxy proxy) {
			final String proxyHost = ProxyUtil.proxyHost(proxy);
			return proxyHost != null ? proxyHost : System.getProperty(protocol + ".proxyHost");
		}

		public int getProxyPort(final String protocol, final Proxy proxy) {
			final int proxyPort = ProxyUtil.proxyPort(proxy);
		...
		public PasswordAuthentication getProxyAuthentication(final ProtexServer server) {
			try {
				final Proxy proxy = server.getProxy();
				final URL serverUrl = new URL(server.getServerUrl());
				final String protocol = serverUrl.getProtocol();
				final String host = getProxyHost(protocol, proxy);
				final int port = getProxyPort(protocol, proxy);
	...
	public static class AnalyzeCommandBuilder extends BlackDuckCommandBuilder {
		...
		public AnalyzeCommandBuilder projectId(final String projectId) {
			this.projectId = projectId;
			return this;
		}

		public AnalyzeCommandBuilder directory(final File directory) {
			this.directory = directory;
			return this;
		}

		public AnalyzeCommandBuilder force(final boolean force) {
			this.force = force;
			return this;
		}
		...
		protected List<String> arguments() {
			final List<String> arguments = super.arguments();
	...
	public static class CodePrintCommandBuilder extends BlackDuckCommandBuilder {
		...
		public CodePrintCommandBuilder codePrintId(final String codePrintId) {
			this.codePrintId = codePrintId;
			return this;
		}

		public CodePrintCommandBuilder path(final String path) {
			this.path = path;
			return this;
		}
	...
	protected BlackDuckCommandBuilder(final String command) {
	...
	public final BlackDuckCommand build() throws BuildToolIntegrationException {
		final ClassLoader loader = processOptions != null ? null : server.getClientLoader();
	 ...
	 */
	public final BlackDuckCommandBuilder connectedTo(final ProtexServer server) {
		this.server = server;
		return this;
	}
	 ...
	 */
	public final void setJava(final File java) {
		this.java = java;
	}
	 ...
	 * Specifies the java executable to use to run the forked process.
	 */
	public final void setBdsToolJavaOptions(final String bdsToolJavaOptions) {
	 ...
	 */
	public final BlackDuckCommandBuilder withMaxHeapSize(final long maxHeapSize) {
	 ...
	 */
	public final BlackDuckCommandBuilder observingProgress(final Observer observer) {
	 ...
	 * Observers are expected to be thread-safe.
	 */
	public final BlackDuckCommandBuilder observingMessages(final Observer observer) {
	 ...
	 * Observers are expected to be thread-safe.
	 */
	public final BlackDuckCommandBuilder observingOutput(final Observer observer) {
	 ...
	 * Observers are expected to be thread-safe.
	 */
	public final BlackDuckCommandBuilder observingScanLog(final Observer observer) {
		addObserver(ClientDriverNotifier.SCAN_LOG, observer);
		return this;
	}

	private void addObserver(final ClientDriverNotifier observableType, final Observer observer) {
	...
	protected List<String> arguments() {
		...
		if (processOptions != null) {
			...
			final String protocol = URI.create(server.getServerUrl()).getScheme();
			final Proxy proxy = server.getProxy();
			...
			final PasswordAuthentication proxyAuthentication = processOptions.getProxyAuthentication(server);
	 ...
	 */
	private void parseBdsToolJavaOptions(final List<String> arguments, final String bdsToolJavaOptions) {
		try {
			final StreamTokenizer tokenizer = new StreamTokenizer(new StringReader(bdsToolJavaOptions));
			tokenizer.quoteChar('"');
			tokenizer.quoteChar('\'');
	 ...
	 */
	protected static String memory(final long bytes) {
		...
		for (int i = suffix.length(); i > 0; --i) {
			final BigInteger size = bigBytes.divide(unit.pow(i));
